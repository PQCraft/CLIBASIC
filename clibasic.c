...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		if (!strcmp(argv[i], "--version") || !strcmp(argv[i], "-v"))
		{
			if (argc > 2)
			{
				printf("Incorrent number of options passed.\n");
				cleanExit();
			}
	...
	if (!runfile)
	{
		prog = fopen("AUTORUN.BAS", "r");
		progFilename = malloc(12);
		strcpy(progFilename, "AUTORUN.BAS");
		if (prog == NULL)
		{
			prog = fopen("autorun.bas", "r");
			strcpy(progFilename, "autorun.bas");
	...
	fchkint:
		...
		if (!inProg)
		{
			...
			if (tmpstr[0] == '\0' || !strcmp(tmpstr, lastcb))
			{
				free(tmpstr);
				goto brkproccmd;
			}
		...
		while (1)
		{
			if (!inProg)
			{
				...
				else if ((conbuf[cp] == ':' && !inStr) || conbuf[cp] == '\0')
				{
					while ((conbuf[cp - cmdl] == ' ') && cmdl > 0)
						cmdl--;
			...
			else
			{
					...
					|| fgrabc(prog, cp) == 0)
				{
					...
					if (feof(prog) || fgrabc(prog, cp) == -1 || fgrabc(prog, cp) == 4 || fgrabc(prog, cp) == 0)
					{
						inProg = false;
						fclose(prog);
						free(progFilename);
...
{
	return ((bfr[pos] >= 'A' && bfr[pos] <= 'Z') || (bfr[pos] >= 'a' && bfr[pos] <= 'z') || (bfr[pos] >= '0' && bfr[pos] <= '9') || bfr[pos] == '_' || bfr[pos] == '$' || bfr[pos] == '%' || bfr[pos] == '&' || bfr[pos] == '!' || bfr[pos] == '~');
}
...
{
	int64_t i2 = 0;
	for (int64_t i3 = i; i3 < j; i3++)
	{
		str2[i2] = fgrabc(file, i3);
		i2++;
	}
	str2[i2] = 0;
}
...
uint8_t getVal(char *tmpinbuf, char *outbuf)
{
	...
	double num3 = 0;
	int numAct;
	if ((isSpChar(inbuf, 0) && inbuf[0] != '-') || isSpChar(inbuf, strlen(inbuf) - 1))
	{
		cerr = 1;
		return 0;
	}
	...
	while (true)
	{
		...
		while (true)
		{
			...
			if ((isSpChar(inbuf, jp) && !inStr && pct == 0) || inbuf[jp] == '\0')
				goto gvbrk;
			jp++;
		}
	gvbrk:
		...
		if (t == 1)
		{
			copyStrSnip(tmp[0], 1, strlen(tmp[0]) - 1, tmp[2]);
		...
		else if (t == 2)
		{
			...
			while (1)
			{
				...
				for (int i = 0; tmp[0][i] != '\0' && p2 == 0; i++)
				{
					...
					if (tmp[0][i] == '^' && !inStr && pct == 0)
					{
						if (!gvchkchar(tmp[0], i))
						{
							return 0;
						}
						p2 = i;
				...
				for (int i = p2 + 1; true; i++)
				{
					if (tmp[0][i] == '"')
						inStr = !inStr;
					if ((isSpChar(tmp[0], i) && i != p2 + 1 && !inStr) || tmp[0][i] == '\0')
					{
						p3 = i;
						break;
					}
				...
				if (dt == 2)
				{
					...
					while (dp && tmp[2][strlen(tmp[2]) - 1] == '0')
						tmp[2][strlen(tmp[2]) - 1] = 0;
					
					if (dp && tmp[2][strlen(tmp[2]) - 1] == '.')
						tmp[2][strlen(tmp[2]) - 1] = 0;
				}
				copyStrSnip(tmp[0], p3, strlen(tmp[0]), tmp[3]);
				if (p1 != 0)
					copyStrSnip(tmp[0], 0, p1, tmp[1]);
...
gvfexit:
	if (dt == 2)
	{
		...
		while (dp && tmp[1][strlen(tmp[1]) - 1] == '0')
			tmp[1][strlen(tmp[1]) - 1] = 0;
		
		if (dp && tmp[1][strlen(tmp[1]) - 1] == '.')
			tmp[1][strlen(tmp[1]) - 1] = 0;
	}
...
bool mkargs()
{
	...
	while (cmd[j] == ' ')
		j++;
	int h = j;
	while (cmd[h] != ' ' && cmd[h] != '=' && cmd[h] != '\0')
		h++;
	copyStrSnip(cmd, h + 1, strlen(cmd), tmpbuf[0]);
	int tmph = h;
	while (cmd[tmph] == ' ' && cmd[tmph] != '\0')
		tmph++;
	...
	for (int i = 0; i <= argct; i++)
	{
		...
		if (debug)
			printf("length of arg[%d]: %d\n", i, argl[i]);

		tmpargs[i][argl[i]] = 0;
	}
	if (argct == 1 && tmpargs[1][0] == '\0')
		argct = 0;
...
uint8_t logictest(char *inbuf)
{
	...
	for (int i = p; true; i++)
	{
		...
		if ((inbuf[i] == '<' || inbuf[i] == '=' || inbuf[i] == '>') && !inStr && pct == 0)
		{
			p = i;
			break;
		}
	...
	tmp[2][tmpp] = '\0';
	if (debug)
		printf("getting tmp[0]... current: {%s}\n", tmp[0]);
	...
	if (t1 == 0)
		return -1;
	if (debug)
		printf("getting tmp[2]... current: {%s}\n", tmp[2]);
...
bool runlogic()
{
	...
	if (!strcmp(tmp[0], "DO"))
	{
		...
		if (dlstackp > 0)
		{
			if (dldcmd[dlstackp - 1])
			{
				dldcmd[dlstackp] = true;
				return true;
			}
		}
		if (itstackp > -1)
	...
	if (!strcmp(tmp[0], "IF"))
	{
		...
		if (itstackp > 0)
		{
			if (itdcmd[itstackp - 1])
			{
				itdcmd[itstackp] = true;
				return true;
			}
		}
		if (dlstackp > -1)
	...
	if (!strcmp(tmp[0], "ELSE"))
	{
		...
		{
			if (debug)
				printf("ELSE: checking itdcmd[itstackp - 1]: [%d]\n", (int)itdcmd[itstackp - 1]);
			if (itdcmd[itstackp - 1])
				return true;
			if (debug)
				printf("ELSE: passed\n");
		...
		{
			if (debug)
				printf("ELSE: checking dldcmd[dlstackp]: [%d]\n", (int)dldcmd[dlstackp]);
			if (dldcmd[dlstackp])
				return true;
			if (debug)
				printf("ELSE: passed\n");
...
cmderr:
	if (cerr != 0)
	{
		...
		if (inProg)
			printf("Error %d on line %d of %s: '%s': ", cerr, progLine, progFilename, cmd);
		else
			printf("Error %d: ", cerr);
		switch (cerr)
		{
